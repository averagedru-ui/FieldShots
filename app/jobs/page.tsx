...
export default function JobsPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#121212]">
      {/* Header */}
      <header className="flex items-center justify-between px-4 pt-safe bg-[#1A1A1A] border-b border-[#2A2A2A] sticky top-0 z-10" style={{ paddingTop: 'max(16px, env(safe-area-inset-top))' }}>
        <Link href="/settings" className="p-2 text-xl">⚙️</Link>
        <h1 className="text-lg font-bold tracking-tight">FieldShots</h1>
        <Link href="/search" className="p-2 text-xl">🔍</Link>
      </header>
        ...
        {jobs.map((job) => (
          <div key={job.id} className="bg-[#1E1E1E] rounded-xl p-4 flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <button onClick={() => confirmDelete(job)} className="mt-1 text-[#555] text-xs px-2 py-1 rounded hover:text-red-400">✕</button>
            </div>
          </div>
        ))}
      </main>
      ...
      <Link
        href="/jobs/new"
        className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-[#4CAF50] flex items-center justify-center text-3xl text-white shadow-lg"
