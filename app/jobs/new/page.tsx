...
export default function NewJobPage() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    const id = await createJob(refCode.trim(), name.trim(), description.trim());
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#121212]">
      <header className="flex items-center gap-3 px-4 bg-[#1A1A1A] border-b border-[#2A2A2A] sticky top-0 z-10" style={{ paddingTop: 'max(16px, env(safe-area-inset-top))', paddingBottom: '12px' }}>
        <Link href="/jobs" className="text-[#4CAF50] text-sm font-semibold">Cancel</Link>
        <h1 className="flex-1 text-center font-bold">New Job</h1>
      ...
      <main className="flex-1 p-4 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="e.g. Roof Inspection — 42 Main St"
          />
        </div>
        <div className="space-y-1.5">
        ...
        <button
          onClick={save}
          disabled={saving}
          className="w-full bg-[#4CAF50] text-white rounded-xl py-4 font-bold text-base disabled:opacity-40"
        >
